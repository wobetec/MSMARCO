...
from src.rankers.monobert import MonoBERT, MonoBatchBERT


class TestMonoBatchBERT(unittest.TestCase):
    def test_init(self):
        dataset = MSMarcoDataset('data/subset_msmarco_train_0')
        dataset.load_data('subset_msmarco_train_0.01_99.pkl')

        monobert = MonoBatchBERT("mixedbread-ai/mxbai-rerank-base-v1")
        self.assertIsInstance(monobert, MonoBatchBERT)
...
class TestMonoBERT(unittest.TestCase):
    def test_init(self):
        dataset = MSMarcoDataset('data/subset_msmarco_train_0')
        dataset.load_data('subset_msmarco_train_0.01_99.pkl')
